...
namespace ConcurrentDictionary.Collection.Tests
{
    ...
    public class ConcurrentDictionaryTests
    {
        ...
        [TestMethod]
        public void ConcurrentDictionary_TryGetValue_Test()
        {
            var stock = new ConcurrentDictionary<string, int>();
            stock.TryAdd("Levis", 100);
            stock.TryAdd("Killer", 35);
            ...
            bool isLoalValuePresent = stock.TryGetValue("Local", out localValue);
            Assert.IsFalse(isLoalValuePresent);
            Assert.IsTrue(localValue == 0);

            int killerValue;
            bool isKillerValuePresent = stock.TryGetValue("Killer", out killerValue);
            Assert.IsTrue(isKillerValuePresent);
            Assert.IsTrue(killerValue == 35);
        ...
        [TestMethod]
        public void ConcurrentDictionary_TryRemove_Missing_Item_Test()
        {
            var stock = new ConcurrentDictionary<string, int>();
            stock.TryAdd("Levis", 100);
        ...
        public void ConcurrentDictionary_TryUpdate_Test()
        {
            ...
            var update1 = stock.TryUpdate("Killer", 100, 35); // key, new value, old value
